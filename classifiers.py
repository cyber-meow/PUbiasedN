...
class Classifier_from2(Classifier):
    ...
    def __init__(self, pi=0.5, *args, **kwargs):
        self.pi = pi
...
class PNClassifier(Classifier_from2):
    ...
    def __init__(self, pu_model=None, *args, **kwargs):
...
class PUClassifier(Classifier_from2):
    ...
    def __init__(self, nn=True, nn_threshold=0, nn_rate=1/100,
                 *args, **kwargs):
...
class Classifier_from3(Classifier):
    ...
    def __init__(self, rho=0.1, *args, **kwargs):
        self.rho = rho
        super().__init__(*args, **kwargs)
...
class WeightedClassifer(Classifier_from3):

    def __init__(self, pp_model, *args, **kwargs):
    ...
    def compute_loss_weighted(self, px, snx, ux, convex):
        ...
        fsnx = self.model(snx.type(dtype))
        fux = self.model(ux.type(dtype))
        fux_prob = self.pp_model(ux.type(dtype))
        fux_prob = fux_prob/torch.mean(fux_prob)*(self.pi+self.pho)
        loss = (
            self.pi * torch.mean(self.basic_loss(fpx, convex))
            + self.pho * torch.mean(self.basic_loss(-fsnx, convex))
...
class PosterirorProbability(object):
    ...
    def compute_loss_density(self, px, snx, ux):
        ...
        loss = fux2_mean - 2*fpx_mean*self.pi - 2*fsnx_mean*self.pho
